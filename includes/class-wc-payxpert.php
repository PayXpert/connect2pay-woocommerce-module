...
class PayXpertMain {
	...
	public function payxpert_process_payment( $order_id, $type, $returnurl, $returnname ) {
		...
		if ( ! empty( $transactionOperation ) && in_array( strtolower( $transactionOperation ), array(
				'sale',
				'authorize'
			) ) ) {
		...
		if ( $result !== false ) {
			...
			update_post_meta( $order_id, '_payxpert_merchant_token', $result->getMerchantToken() );
			update_post_meta( $order_id, '_payxpert_customer_url', $c2pClient->getCustomerRedirectURL( $result ) );
	...
	private function recordCardToken( $transaction, $order ) {
		...
		if ( $paymentMeanInformation !== null && $paymentMeanInformation->getCardToken() !== null ) {
			...
			if ( ! $token->save() ) {
				wc_add_notice( 'Error saving the card token from PayXpert response', 'error' );
			}
	...
	public function payxpert_receipt_page( $order_id ) {
		...
		//display the form
		?>
        <iframe id="payxpert_for_woocommerce_iframe" src="<?php echo $payxpert_customer_url; ?>" width="100%"
                height="700" scrolling="no" frameborder="0" border="0" allowtransparency="true"></iframe>
		<?php
	}
	...
	public function seamless_credit_card_process_payment( $order_id ) {
		...
		update_post_meta( $order_id, '_payxpert_payment_id', $_POST['paymentstatus'] );
